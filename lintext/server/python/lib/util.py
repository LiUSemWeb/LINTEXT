...
def run_exp(fb:FitBert = None, resdir='res/', task_name='docred', dset='dev', doc=0, num_blanks=2, num_passes=1, nonlinearity=None, pooling=None, use_ent=True, top_k=0, skip=[], model='bert-large-cased', start_at=0):
...
def run_many_experiments(task_name, dset, rel_info, nonlins, poolers, scorers, num_blanks, docnum=-1, num_passes=1, max_batch=2000, use_ent=True, skip=[], stopfile='', model=None, start_at=0):
    # import time
    if num_blanks == 0:
        poolers = [None]
    with torch.no_grad():
        ...
        all_all_scores = dict()
        for p_doc in run_exp(fb, task_name=task_name, dset=dset, doc=docnum, num_blanks=num_blanks, num_passes=num_passes, use_ent=use_ent, skip=skip, model=model_name, start_at=start_at, top_k=0):
            all_scores = dict()
